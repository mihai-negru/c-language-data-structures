...
#define MLIST(ID, T)                                                           \
  ...
  ID##_mlist_t ID##_mlist(ID##_compare_func cmp, ID##_free_func frd) {         \
    ...
    return self;                                                               \
                                                                               ...
                                                                               \
  ID##_mlist_node_t ID##_internal_mlist_node(T *const data) {                  \
...
#define MLIST_FIND_NODE(ID, T)                                                 \
  ID##_mlist_node_t ID##_internal_mlist_find_node(                             \
      const ID##_mlist_ptr_t *const self, T *const data) {                     \
    if ((self == NULL) || (self->head == NULL)) {                              \
      return NULL;                                                             \
    }                                                                          \
                                                                               \
  ...
  merr_t ID##_mlist_swap(const ID##_mlist_t self, T fst, T snd) {              \
    if (self == NULL) {                                                        \
      return M_NULL_INPUT;                                                     \
                                                                               ...
                                                                               \
    ID##_mlist_node_t fst_node = ID##_internal_mlist_find_node(self, &fst);    \
                                                                               \
    if (fst_node == NULL) {                                                    \
      return M_NOT_FOUND;                                                      \
    }                                                                          \
                                                                               \
    ID##_mlist_node_t snd_node = ID##_internal_mlist_find_node(self, &snd);    \
                                                                               \
    if (snd_node == NULL) {                                                    \
      return M_NOT_FOUND;                                                      \
    }                                                                          \
  ...
  merr_t ID##_mlist_change(const ID##_mlist_t self, T base, T new) {           \
    if (self == NULL) {                                                        \
      return M_NULL_INPUT;                                                     \
                                                                               ...
                                                                               \
    ID##_mlist_node_t base_node = ID##_internal_mlist_find_node(self, &base);  \
                                                                               \
    if (base_node == NULL) {                                                   \
      return M_NOT_FOUND;                                                      \
    }                                                                          \
  ...
  merr_t ID##_mlist_push(const ID##_mlist_t self, T data) {                    \
    if (self == NULL) {                                                        \
      return M_NULL_INPUT;                                                     \
                                                                               ...
                                                                               \
    ID##_mlist_node_t self_node = ID##_internal_mlist_node(&data);             \
                                                                               \
    if (self_node == NULL) {                                                   \
