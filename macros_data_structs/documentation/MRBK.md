...
In order to use the strcutures and the whole api, you need to clone the "**m_rbk.h**" file into your project. The for including you just need to simply include it.

```c
  #include "path_to_file/m_rbk.h"
```
...
So the type chosen is int, and I have assinged the name of *doc*. For the creating of an rbk tree, we need two things:

* A compare function for the type, because the rbk tree is an ordered structure and we need to arrange the data in a specified order.
