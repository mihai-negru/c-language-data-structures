...
int main(void) {
  ...
  test_mdlist_t strings = test_mdlist(&compare_ptr_str, &free_str);
  ...
  for (size_t i = 0; i < str_num; ++i) {
    ...
    err = test_mdlist_push(strings, ptr_data);
  ...
  test_mdlist_traverse(strings, &print_str);
  printf("\n");

  char *find;
  if (test_mdlist_find_idx(strings, 4, &find) == M_OK) {
    printf("String from pos 4 is %s\n", find);
  }

  test_mdlist_free(&strings);
