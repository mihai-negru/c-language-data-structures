...
int main(void) {
  ...
  test_mbst_t data_base = test_mbst(&compare_student, &free_student);
  ...
  for (int i = 0; i < number_of_students; ++i) {
    ...
    err = test_mbst_push(data_base, st);
  ...
  printf("In the data base are %lu students\n\n", test_mbst_size(data_base));

  test_mbst_traverse_inorder(data_base, &print_student);
  printf("\n\n");

  fclose(fout);

  test_mbst_free(&data_base);
