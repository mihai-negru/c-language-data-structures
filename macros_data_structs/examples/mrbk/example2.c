...
int main(void) {
  ...
  test_mrbk_t data_base = test_mrbk(&compare_student, &free_student);
  ...
  for (int i = 0; i < number_of_students; ++i) {
    ...
    err = test_mrbk_push(data_base, st);
  ...
  printf("In the data base are %lu students\n\n", test_mrbk_size(data_base));

  test_mrbk_traverse_inorder(data_base, &print_student);
  printf("\n\n");

  fclose(fout);

  test_mrbk_free(&data_base);
