...
int main(void) {
  ...
  test_mavl_t my_tree = test_mavl(&compare_int, NULL);
  ...
  for (size_t i = 1; i <= number_of_elem; ++i) {
    ...
    err = test_mavl_push(my_tree, data);
  ...
  if (err == M_OK) {
    ...
    int anc;
    if (test_mavl_lca(my_tree, root_pred, root_succ, &anc) == M_OK) {
  ...
  for (size_t i = 0; i < 10; ++i) {
    ...
    if (test_mavl_root(my_tree, &del) == M_OK) {
      printf("Removing %d value from AVL\n", del);

      err = test_mavl_pop(my_tree, del);
  ...
  test_mavl_traverse_inorder(my_tree, &print_int);
  printf("\n\n");

  fclose(fout);
  test_mavl_free(&my_tree);
