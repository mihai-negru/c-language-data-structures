...
int main(void) {
    ...
    list_t *strings = create_list(&compare_string, NULL, MAX_STRING_SIZE); 
    ...
    for (size_t i = 0; i < str_num; ++i) {
        ...
        err = list_insert(strings_2, toptr(ptr_data));
    ...
    printf("Print strings list by first method:\n");
    list_traverse(strings, &print_string);
    printf("\n\n");

    printf("Print strings list by second method:\n");
    list_traverse(strings_2, &print_str);
    printf("\n");

    char * const * find_1 = list_find_index(strings_2, 4);
