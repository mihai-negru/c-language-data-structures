...
int main(void) {
    ...
    rbk_tree_t *data_base = create_rbk(&compare_student, &free_student, sizeof(student_t));
    ...
    for (int i = 0; i < number_of_students; ++i) {
        ...
        err = rbk_insert(data_base, toptr(st));
    ...
    printf("In the data base are %lu students\n\n", get_rbk_size(data_base));

    rbk_traverse_inorder(data_base, &print_student);
    printf("\n\n");

    printf("After two years our students got older so let's change their age\n\n");
    rbk_traverse_inorder(data_base, &two_years_passed);    
